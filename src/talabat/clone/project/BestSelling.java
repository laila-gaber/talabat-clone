...
//import static talabat.clone.project.owner;
...
Border pborder = BorderFactory.createLineBorder(Darkorange,2);   
    ...
    public void disblayBestselling()
    {
        ...
        mealsFlag = BestsellingIn("Meales",l3,l4,l33,l44,l5);
        if(mealsFlag==0)
                {
       ...
       dessertsflag = BestsellingIn("Desserts",l7,l8,l77,l88,l9);
       if(dessertsflag==0)
                {
        ...
        drinksflag = BestsellingIn("Drinks",l11,l12,l111,l122,l13);
        if(drinksflag==0)
                {
    ...
    public int BestsellingIn(String type , JLabel l1 , JLabel l2, JLabel l3, JLabel l4, JLabel l5)
    {
        ...
        for(int i=1 ; i<=getOrder.numberOfOrder ; i++) //int i=1 //sara
        { if(getOrder.type_food[i]==type)
            {
                
                if(getOrder.number_food[i]>qntty )
                {
                    l1.setText(getOrder.name_food[i]);
                    l2.setText(String.valueOf("EGP " +getOrder.price_food[i]));
                    indx=i;
                    qntty=getOrder.number_food[i];
                    l5.setText(String.valueOf(getOrder.number_food[i]+" Sold Out From"));
                    //To make sure that we have sales in this tybe of food
                    flag =1;
                }
                
                else if( getOrder.number_food[i] == qntty )
                {
                    l1.setText(getOrder.name_food[i]);
                    l2.setText(String.valueOf("EGP " +getOrder.price_food[i]));
                    l3.setText(getOrder.name_food[indx]);
                    l4.setText(String.valueOf("EGP " +getOrder.price_food[indx]));
                    l5.setText(String.valueOf(getOrder.number_food[i]+" Sold Out From"));
                    flag =1;
                }
